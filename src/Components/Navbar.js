...
export default function Navbar(props) {
    ...
    return (
        <>
            ...
            {/* Mobile */}
            <nav className="w-full mx-auto bg-[#0f172a]  shadow relative z-20">
                <div className="justify-between container px-6 h-16 flex items-center lg:items-stretch mx-auto">
                    <div className="flex items-center">
                        ...
                        <ul className="pr-32 xl:flex hidden items-center h-full space-x-4">
                            ...
                            onClick={() => setDeliverables(!deliverables)}>
                                {deliverables ? (
                                    <ul className="bg-white shadow rounded py-1 left-0 mt-16 -ml-4 absolute w-50 max-h-[75vh] overflow-auto top-2">
                                        {cities.map(city => (
                                            (city.id != 0) && <li className="text-sm text-gray-800 hover:text-orange-500 p-2" key={city.id}>
                                                <span onClick={()=>cityToNavigate(city.name)}>
                                                    {city.name}
                                                </span>
                                            </li>
                    ...
                    <div className="visible xl:hidden flex items-center">
                        <div>
                            <div id="menu" className="text-slate-400" onClick={() => setShow(!show)}>
